...
var mainUrl = "http://localhost:8080/nzj/";
...
function getDateShow() {
	...
	if(date.getHours() <= 12 && getCookie('username') != null) {
		$('.topLeft').html('上午好:' + getCookie('username'));
	} else if(date.getHours() <= 24 && getCookie('username') != null) {
		$('.topLeft').html('下午好:' + getCookie('username'));
	} else {
		$('.topLeft').html('');
	}
