...
public class TraineeDaoImp implements TraineeDao {
	...
	@Override
	public List getUnBindTraineeList(Long uid, Integer start, Integer limit) {
		try {
			Session session = HibernateSessionFactory.getSession();
			Transaction ts = session.beginTransaction();

			Query query = session
					.createQuery("from Trainee where userId=? and bind=0");
	...
	public long getUnBindTraineeCount(Long uid) {
		try {
			...
			Query query = session
					.createQuery("select count(id) from Trainee where userId=? and bind=0");
			query.setParameter(0, uid);
			query.setMaxResults(1);
			long count = (long) query.uniqueResult();
	...
	public long getTraineeCountByUserId(Long userId) {
		try {
			...
			Query query = session
					.createQuery("select count(id) from Trainee where userId=?");
